extends Node3D
class_name BridgeEnvironment
## A single self-contained RL environment: one voxel terrain, two anchor cube
## stacks, a BuildSystem and an RLAgent. Instanced N times by voxel_terrain.gd
## (each at its own X/Z world offset) so one Godot process hosts a vector of
## independent environments. All child nodes are built relative to this node, so
## the offset set on its `position` carries through to terrain, cubes and placed
## blocks.
...
# Which environment this is in the layout. Only env 0 draws the HUD, so N
# environments don't stack N overlapping HUDs on screen.
var env_index := 0
...
# RL wiring (set up once in setup()).
var _build_system: Node3D
var _controller: Node3D

## Build the BuildSystem + RLAgent for this environment. Called by the
## orchestrator after this node is added to the tree at its layout offset.
func setup() -> void:
	generate_world()
	...
	_build_system.show_hud = env_index == 0
